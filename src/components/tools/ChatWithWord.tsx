...
import { extractTextFromWord, convertWordToHtml } from "@/lib/word";
import { chatWithAI } from "@/app/actions-chat";
import { Send, Loader2, User, Bot, Sparkles, X, FileText } from "lucide-react";
...
export default function ChatWithWord() {
    ...
    const handleFileSelect = async (files: File[]) => {
        if (files.length > 0) {
            ...
            try {
                ...
                setWordText(text);
                setWordHtml(html);
                setMessages([{ role: 'assistant', content: "Hello! I've analyzed your Word document. What would you like to know about it?" }]);
            } catch (error: any) {
                console.error("Extraction error:", error);
                alert("Failed to analyze Word document. Please ensure it's a valid .docx file.");
                setFile(null);
            } finally {
    ...
    const handleSend = async (e?: React.FormEvent) => {
        ...
        if (!input.trim() || !wordText || isLoading) return;
        ...
        try {
            // Reusing chatWithPDF action since it just takes text and history
            const response = await chatWithAI(wordText, [...messages, userMessage], 'DOCX');
    ...
    return (
        <ToolLayout
            ...
            description="The most advanced AI document assistant for Word. Upload a .docx file and instantly summarize, extract data, or ask complex questions."
        >
            <div className="space-y-12">
                {!file && (
                    <FileUpload
                        accept=".docx"
                        label="Select Word Document to Chat With"
                ...
                {file && (
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:h-[750px] min-h-[600px] animate-in fade-in zoom-in-95 duration-500">
                        ...
                        <div className="bg-zinc-50 border border-zinc-100 rounded-[2.5rem] overflow-hidden flex flex-col h-full shadow-inner relative">
                            <div className="bg-white border-b border-zinc-100 p-4 sm:p-6 flex items-center justify-between shadow-sm">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <h3 className="font-black text-zinc-950 text-sm">AI Word Assistant</h3>
                                        <p className="text-[10px] text-zinc-600 font-black uppercase tracking-widest">Powered by SendNow AI</p>
                                    </div>
                                </div>
                                <button
                                    onClick={() => { setFile(null); setWordHtml(null); setMessages([]); }}
                            ...
                            <form onSubmit={handleSend} className="p-4 sm:p-6 bg-white border-t border-zinc-100 shadow-sm">
                                <div className="relative">
                                    <input
                                        ...
                                        placeholder="Ask specialized questions about this Word doc..."
                                    ...
                                    <button
                                        type="submit"
                                        disabled={!input.trim() || isLoading || !wordText}
                        ...
                        <div className="bg-white border border-zinc-200 rounded-[2.5rem] overflow-hidden flex flex-col shadow-xl">
                            ...
                            <div className="flex-grow overflow-auto p-12 bg-white prose prose-sm max-w-none">
                                {wordHtml ? (
                                    ...
                                    <div className="h-full flex flex-col items-center justify-center text-zinc-400 gap-4">
                                        ...
                                        <p className="font-bold">Processing preview...</p>
                                    </div>
                                )}
                            </div>
                        </div>
